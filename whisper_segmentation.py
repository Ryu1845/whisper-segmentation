...
from pywhisper.decoding import DecodingOptions, DecodingResult
from pywhisper.tokenizer import LANGUAGES, get_tokenizer
from pywhisper.utils import (
    exact_div,
    format_timestamp,
)
import ffmpeg
import pywhisper as whisper
from pywhisper.model import Whisper
from tap import Tap


class Args(Tap):
    audio: str  # Your audio file
    directory: str = "segments"  # Where the output segments will be written to


# this function is modified from the openai/whisper implementation
def transcribe(
    model: "Whisper",
...
if __name__ == "__main__":
    ...
    result = transcribe(model, args.audio, verbose=True)

    segment_dir = Path(args.directory)
